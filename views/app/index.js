...
const getTodos = async () => {
	try {
		...
		if (todos.length === 0) {
			...
			li.classList.add('flex', 'felx-row');
			li.innerHTML = `
							<div class="group grow flex flex-row justify-between">
								<i
									class="delete-icon w-12 md:w-14 hidden group-hover:flex group-hover:justify-center group-hover:items-center cursor-pointer bg-red-500 origin-left">
									<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 md:h-7 md:w-7 text-white"
										fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
										<path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
									</svg>
								</i>
								<p class="p-4 break-words grow">No tienes nada compai!</p>
							</div>
							<i
								class="check-icon w-12 md:w-14 flex justify-center items-center cursor-pointer border-l border-slate-300 dark:border-slate-400 hover:bg-green-300 transition duration-300 easy-in-out">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 md:h-7 md:w-7 text-green-600"
									fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
	...
	checkBtn.forEach(async element => {
		if (event.target === element.parentElement || event.target === element.parentElement.parentElement || event.target === element) {
			...
			if (!todo.classList.contains('line-through')) {
				...
				await axios.path(`/api/todos/${id}`, { checked: false });
